...
public class CheckpointHandler : MonoBehaviour
{
    ...
    void Generate()
    {
        ...
        for (int i = 0; i < checkpointCount; i++)
        {
            ...
            var cp = Instantiate(checkpoint, (Vector3)path.Spline.EvaluatePosition(t) + path.transform.position, Quaternion.identity, transform);
        ...
        for (int i = 0; i < checkpointCount; i++)
        {
            for (int j = 0; j < checkpoints[i].Count; j++)
            {
                ...
                {
                    if (Vector3.Distance(checkpoints[i][j].transform.position, checkpoints[i][k].transform.position) < mergeDistance)
                    {
                        checkpoints[i][j].transform.position = (checkpoints[i][j].transform.position + checkpoints[i][k].transform.position) / 2;
        ...
        for (int i = 1; i < path.Splines.Count; i++)
        {
            ...
            for (int j = 0; j < checkpointCount; j++)
            {
                ...
                if (start < end && (t > start && t < end) || start > end && (t > start && t < 1 || t > 0 && t < end))
                {
                    ...
                    Gizmos.DrawWireSphere((Vector3)path.Splines[i].EvaluatePosition((t - start) / (end - start)) + path.transform.position, mergeDistance);
                }
            }
        }
    }
